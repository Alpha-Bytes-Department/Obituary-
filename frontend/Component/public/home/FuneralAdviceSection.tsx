...
/**
 * Renders the funeral advice and grief support section.
 *
 * @returns {JSX.Element} The static advice section.
 */
    ...
    <section className="space-y-8 px-0 py-0">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl font-sans text-[1.02rem] leading-7 text-slate-600 sm:text-[1.08rem]">
          ...
          loss, written as practical guidance families can return to whenever
...
function ArticlePreviewCard({
  ...
  return (
    <button
      ...
      className="group h-full overflow-hidden rounded-md border border-[#d8c8b0] bg-white text-left shadow-[0_14px_32px_rgba(15,23,42,0.08)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(15,23,42,0.12)]"
    >
      <div className="relative aspect-4/3 overflow-hidden bg-[#e9e3d8]">
        <Image
          src={article.image}
          alt={article.title}
          fill
      ...
      <div className="space-y-3 border-t border-[#e7ddd1] p-5">
        <p className="font-sans text-xs font-semibold uppercase tracking-[0.28em] text-[#bb8b37]">
          {article.label}
        </p>
        <h3 className="font-heading text-[1.7rem] font-semibold leading-tight tracking-[-0.03em] text-[#284c73] sm:text-[1.8rem]">
          {article.title}
        </h3>
        <p className="font-sans text-[0.98rem] leading-7 text-slate-700">
          {article.summary}
        </p>
      </div>
    </button>
...
function FaqRail({
  ...
  return (
    <div className="flex h-full flex-col rounded-md px-0 py-0">
      <div className="space-y-0">
        <p className="mb-4 font-heading text-xl font-semibold tracking-[-0.02em] text-[#2a4d74]">
          {title}
        </p>
        <div className="space-y-0">
          {articles.map((article) => (
            ...
            >
              <span className="font-heading text-[1.02rem] leading-7 font-medium tracking-[-0.015em] text-[#31343a] transition group-hover:text-[#284c73]">
                {article.title}
              </span>
...
function SponsoredCardView({ card }: { card: SponsoredCard }) {
  return (
    <article className="relative flex min-h-112 w-full max-w-full min-w-0 overflow-hidden rounded-md border border-[#e6c08a] bg-[#f3d8aa] text-left shadow-[0_14px_32px_rgba(15,23,42,0.09)] transition duration-500">
      <Image
        src={card.image}
        alt={card.title}
        fill
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-5 sm:p-6">
        <div className="w-full max-w-full space-y-3 rounded-md bg-black/12 p-4 text-white backdrop-blur-[1px] sm:max-w-88 sm:p-5">
